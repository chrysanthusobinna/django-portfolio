...
This script tests the CV parsing service without external dependencies
"""
...
def test_cv_parser():
    ...
    expected_linkedin = "https://linkedin.com/in/johndoe"
    
    success = True
    if contact.get('email') != expected_email:
        print(f"❌ Email extraction failed: expected {expected_email}, got {contact.get('email')}")
        success = False
    else:
        print("✅ Email extraction successful")
    
    if contact.get('phone') != expected_phone:
        print(f"❌ Phone extraction failed: expected {expected_phone}, got {contact.get('phone')}")
        success = False
    else:
        print("✅ Phone extraction successful")
    
    if contact.get('linkedin') != expected_linkedin:
        print(f"❌ LinkedIn extraction failed: expected {expected_linkedin}, got {contact.get('linkedin')}")
        success = False
    else:
        print("✅ LinkedIn extraction successful")
    
    if about and "software engineer" in about.lower():
        print("✅ Summary extraction successful")
    else:
        print(f"❌ Summary extraction failed: got {about}")
        success = False
